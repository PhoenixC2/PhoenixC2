...
class ListenerTest(unittest.TestCase):
    ...
    def test3_start_listener(self):
        ...
        print(listener_id)
        response = self.session.post(self.url + listener_id + "/start" + json_param, data={'id': 1})
        self.assertEqual(response.status_code, 200)
    
    def test4_restart_listener(self):
        print("Restarting Listener")
        response = self.session.post(self.url + listener_id + "/restart" + json_param, data={'id': 1})
        self.assertEqual(response.status_code, 200)
    
    def test5_stop_listener(self):
        print("Stopping Listener")
        response = self.session.post(self.url + listener_id + "/stop" + json_param, data={'id': 1})
        self.assertEqual(response.status_code, 200)
    
    
    def test6_remove_listener(self):
        print("Removing Listener")
        response = self.session.delete(self.url + listener_id + "/remove" + json_param, data={'id': 1})
        self.assertEqual(response.status_code, 200)
